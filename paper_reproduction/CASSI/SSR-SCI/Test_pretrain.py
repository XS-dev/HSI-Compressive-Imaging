...
## Model param
parser = argparse.ArgumentParser(description="PyTorch Spectral Snapshot Reconstruction")
...
parser.add_argument("--scale", default='L', type=str, help='Model scale')
...
if __name__ == "__main__":
    ...
    if opt.scale == 'S' or opt.scale == 's':
        opt.stage = 3
    elif opt.scale == 'M' or opt.scale == 'm':
        opt.stage = 6
    elif opt.scale == 'L' or opt.scale == 'l':
        opt.stage = 9
    model = Net(opt).cuda()
    ...
    with torch.no_grad():
        checkpoint = torch.load("./Checkpoint_pretrain/model_%s.pkl" % opt.scale)
